...
const getWeatherUrlData = async (lat, long) => {
  try {
    const url = `https://api.weatherapi.com/v1/current.json?key=323e19f8a59c4cff80861201240206&q=${lat},${long}&aqi=yes`;
    const rawResponse = await fetch(url);
    const resultResponse = await rawResponse.json();
